...
隨著車輛數目增加，傳統停車場管理面臨人力成本高昂與管理效率低落的問題，尤其「無障礙車位遭非法佔用」更是長期難以根治的痛點。本專案旨在開發一套「智慧停車場管理系統」，透過軟硬體整合達成以下三大目標：

1. 落實無障礙車位管理： 結合影像辨識與物聯網技術，實現全自動化的違規偵測與現場警示，有效遏止非授權車輛佔用身障車位，保障身障人士權益。

2. 提升營運效率： 取代傳統人工巡視與收費模式，透過車牌辨識 (LPR) 實現無接觸進出場與自動計費，降低管理成本。

3. 優化使用者體驗： 整合行動應用程式 (App) 與網頁後台，提供駕駛即時車位查詢服務，並賦予管理員遠端監控與數據分析的能力。
...
本系統採 RESTful API 架構，所有資料交換皆使用標準 JSON 格式。為確保傳輸安全與資料正確性，所有硬體請求皆需附帶 Token 驗證 與 時間戳記。

##### 2.1 硬體狀態回報 API (ESP32 -> Server)

用途： 地端感測器 (ESP32) 定時或偵測到狀態改變時，呼叫此 API 更新資料庫，並同時接收伺服器的控制指令。

Endpoint: POST /api/hardware/update_status

Request Body (JSON) - ESP32 上傳:

    {
      "auth_token": "sk_esp32_secret_12345", // [安全] 裝置驗證金鑰，防止偽造數據
      "spot_id": "Disable_01",               // 車位編號
      "distance": 30.5,                      // 超音波測距數值 (cm)
      "is_occupied": true,                   // 佔用狀態判定 (true=有車)
      "timestamp": 1703088000                // [校時] Unix 時間戳記，確保事件順序正確
    }

Logic (後端處理邏輯):

1. 驗證身分： 檢查 auth_token 是否合法。

2. 違規判斷： 若 is_occupied: true 且 spot_id 為無障礙車位，後端立即比對白名單。

3. 觸發流程： 若判斷為違規，伺服器將在 Response 中回傳警示指令。

##### 2.2 硬體控制指令 API (Server -> ESP32)

用途： 伺服器透過 HTTP Response 回傳控制指令，指揮 ESP32 執行硬體動作（如：亮燈、鳴叫、開閘）。

機制： 採用 Response-Action 模式（ESP32 上傳狀態後，讀取伺服器的回應來決定動作）。

Response Body (JSON) - 伺服器回傳:

    {
      "status": "success",
      "server_time": 1703088001,      // 供 ESP32 校正 RTC 時間
      "command": "TRIGGER_ALARM",     // 控制指令：NONE (無動作), TRIGGER_ALARM (警報), OPEN_GATE (開閘)
      "message": "Violation detected. Alarm activated." // 系統紀錄訊息
    }

##### 2.3 使用者車位查詢 API (App -> Server)

用途： 手機 App 查詢即時車位資訊，支援針對「一般車位」與「無障礙車位」的分類統計。
...
Response Body (JSON):
      ...
      {
          "last_updated": "2024-01-01 12:05:30",
          "summary": {
          "total_spots": 50,
          "available_general": 10,       // 一般車位剩餘數
          "available_disabled": 2        // 無障礙車位剩餘數 (重點資訊)
          },
          "spots_detail": [
          {
             "id": "A01",
          ...
          },
          {
             "id": "Disable_01",
             "type": "disabled",
             "status": "violation"        // 狀態：empty, occupied, violation (違規佔用)
          }
        ]
     }    
